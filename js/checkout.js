...
function renderCheckoutSummary() {
  ...
  document.getElementById('checkout-items').innerHTML = cart.map(item => {
    ...
    return `
    <div style="display:flex;justify-content:space-between;gap:10px;align-items:center;font-size:0.85rem;padding:8px 0;border-bottom:1px solid #f0ebe5">
      <div style="display:flex;align-items:center;gap:10px;min-width:0;flex:1">
        <img
          src="${safeImageUrl || 'https://placehold.co/80x80/F5F1ED/333333?text=DURI'}"
          alt="${item.name}"
          style="width:42px;height:42px;border-radius:8px;object-fit:cover;border:1px solid #e8e3dd;flex-shrink:0"
          onerror="this.src='https://placehold.co/80x80/F5F1ED/333333?text=DURI'"
        />
        <span style="color:#374151;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${item.name} <span style="color:#9ca3af">x${item.qty}</span></span>
      </div>
      <span style="font-weight:700;white-space:nowrap">${formatPrice(item.price * item.qty)}</span>
    </div>
  `}).join('')
...
if (custError) console.warn('Lưu customer lỗi (không ảnh hưởng đơn hàng):', custError)
    ...
    // Lưu đơn vừa đặt để có thể xem nhanh ngay trên modal thành công
    const lastPlacedOrder = {
      ...
      items: cart.map(item => ({
        ...
        qty: item.qty,
        price: item.price,
        image_url: resolveProductImage(item.id, item.image_url)
      }))
    }
    window._lastPlacedOrder = lastPlacedOrder
    localStorage.setItem('duri_last_order', JSON.stringify(lastPlacedOrder))
...
async function copyOrderCode() {
  ...
  if (!code) {
    showToast('Không tìm thấy mã đơn để sao chép', 'error')
    return
  }
  ...
  try {
    ...
    console.error('Lỗi sao chép mã đơn:', err)
    showToast('Không thể sao chép mã đơn hàng', 'error')
  }
}

function getLastPlacedOrder() {
  if (window._lastPlacedOrder) return window._lastPlacedOrder
  ...
  try {
    ...
    window._lastPlacedOrder = parsed
    return parsed
  } catch (err) {
    console.error('Lỗi đọc đơn hàng vừa đặt:', err)
    return null
  }
}

function goToOrderDetails() {
  const code = window._lastOrderCode || getLastPlacedOrder()?.order_code
  if (!code) {
    showToast('Không tìm thấy mã đơn hàng', 'error')
    return
  }
  window.location.href = `order-details.html?code=${encodeURIComponent(code)}`
}
